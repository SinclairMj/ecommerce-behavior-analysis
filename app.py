...
if uploaded_file is not None:
  try:
    ...
    if missing_cols:
      st.sidebar.error(
          f"❌ 上传失败！CSV 缺少核心字段: {missing_cols}。请确保包含"
...
if filtered_df.empty:
  ...
  view_mode_yoy = st.radio(
      "选择年度业绩查看形式:", ["折线图", "柱状图", "数据表格"], horizontal=True, key="yoy_view"
  )
  ...
  view_mode_cat = st.radio(
      "选择跨年商品类目查看形式:", ["柱状图", "数据表格"], horizontal=True, key="cat_view"
  )
  ...
  view_mode_mom = st.radio(
      "选择月度趋势查看形式:", ["折线图", "柱状图", "数据表格"], horizontal=True, key="mom_view"
  )
  ...
  view_mode_season = st.radio(
      "选择季节性偏好查看形式:", ["柱状图", "数据表格"], horizontal=True, key="season_view"
  )
